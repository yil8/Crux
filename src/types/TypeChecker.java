...
public class TypeChecker implements CommandVisitor {
	...
	private void checkForBadArgument(FunctionDefinition node)
	{
		...
		for (Type arg : args) {
			if (arg instanceof VoidType) {
				errorBuffer.append("TypeError(" + node.lineNumber() + "," + node.charPosition() + ")");
				errorBuffer.append("[" + "Function " + func.name() + " has a void argument in position " + pos + ".]\n");
			} else if (arg instanceof ErrorType) {
				ErrorType erg = (ErrorType)arg;
				errorBuffer.append("TypeError(" + node.lineNumber() + "," + node.charPosition() + ")");
				errorBuffer.append("[" + "Function " + func.name() + " has an error in argument in position " + pos + ": " + erg.getMessage() + "]\n");
			}
			pos++;
		}
	}
	...
	private void put(Command node, Type type)
	{
		if (type instanceof ErrorType) {
			errorBuffer.append("TypeError(" + node.lineNumber() + "," + node.charPosition() + ")");
			errorBuffer.append("[" + ((ErrorType)type).getMessage() + "]" + "\n");
		}
	...
	public void visit(FunctionDefinition node) {
		...
		if (!(currentReturnType() instanceof VoidType) && !allPathsReturn(node.body())) {
			errorBuffer.append("TypeError(" + node.lineNumber() + "," + node.charPosition() + ")");
			errorBuffer.append("[Not all paths in function " + currentFunction.name() + " have a return]");
		}
